...
const updateDiaryEntry = async (
  ...
  if (!response.ok) {
    throw new Error(
      data.message || "Failed to update diary entry"
    );
  }
...
const getDiaryEntryDetail = async (id: string) => {
  ...
  if (!response.ok) {
    throw new Error(data.message || "Failed to fetch diary entry detail");
  }

  return data;
};

const getDiaryEntriesByCategory = async (category: string) => {
  const token = await SecureStore.getItemAsync("token");
  ...
  const response = await fetch(
    `${API_URL}/api/diaryEntries/category/${category}`,
    {
      method: "GET",
      headers: {
        Authorization: `Bearer ${token}`,
  ...
  if (!response.ok) {
    throw new Error(data.message || "Failed to fetch diary entries by category");
  }
